...
func withSpan(spanName string, snapshots []Snapshot, absStartTime time.Time, deployment *docker.Deployment, fn func() error) ([]Snapshot, error) {
	startTime := time.Now()
	if err := fn(); err != nil {
	...
	snapshots = append(snapshots, snapshotStats(spanName, deployment, absDuration, duration)...)
...
func runTest(testName string, builder *docker.Builder, deployer *docker.Deployer, seed int64) ([]Snapshot, error) {
	...
	defer deployment.Deployer.Destroy(deployment, false)

	snapshots := snapshotStats("startup", deployment, 0, 0)
	runner := instruction.NewRunner(testName, false, true)

	absStartTime := time.Now()
	numUsers := 10

	snapshots, err = withSpan("create_users", snapshots, absStartTime, deployment, func() error {
		// make N users
	...
	numRooms := 50
	snapshots, err = withSpan("create_rooms", snapshots, absStartTime, deployment, func() error {
		// make M rooms
	...
	snapshots, err = withSpan("join_rooms", snapshots, absStartTime, deployment, func() error {
		var hs b.Homeserver
		...
		for i := 0; i < numJoins; i++ {
			...
			roomI := int(rnd.NormFloat64()*stddev + (float64(numRooms) / 2.0))
	...
	numEvents := 100
	snapshots, err = withSpan("send_msgs", snapshots, absStartTime, deployment, func() error {
		var hs b.Homeserver
		roomMap := make(map[string]b.Room)
	...
	snapshots, err = withSpan("initial_syncs", snapshots, absStartTime, deployment, func() error {
	...
	snapshots, err = withSpan("display_name_change", snapshots, absStartTime, deployment, func() error {
		return runner.RunInstructions(runOpts, syncInstructions)
	})
	if err != nil {
		return nil, err
	...
	snapshots, err = withSpan("incremental_sync", snapshots, absStartTime, deployment, func() error {
		return runner.RunInstructions(runOpts, syncInstructions)
	})
	if err != nil {
		return nil, err
