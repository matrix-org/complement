...
// NewWaiterGroup returns a struct similar to sync.WaitGroup, and is a "sibling" to Waiter
// A WaiterGroup can start with a specific count, which then can only go down with every
// WaiterGroup.Done invocation.
func NewWaiterGroup(count uint) *WaiterGroup {
	return &WaiterGroup{
		Mutex:    &sync.Mutex{},
...
// Waits until all calls to Done are finalized, or fails the test after `timeout`.
func (wg *WaiterGroup) WaitAll(t *testing.T, timeout time.Duration) {
	t.Helper()
	select {
	case <-wg.finishCh:
		return
	case <-time.After(timeout):
		t.Fatalf("WaitAll: timed out after %s.", timeout)
	}
}

// Calls one task "done" on this group,
// will panic if more tasks have called done than it's internal count.
func (wg *WaiterGroup) Done() {
	wg.Lock()
	defer wg.Unlock()
	if wg.count == 0 {
		panic("More tasks were done with waiterGroup than it originally counted for")
	}
	wg.count--
	if wg.count == 0 {
		close(wg.finishCh)
