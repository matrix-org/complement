...
func TestCanKickJoinedUser(t *testing.T) {
	deployment := complement.Deploy(t, 1)
	defer deployment.Destroy(t)
	...
	// Alice kicks bob from room
	alice.MustKickFromRoom(t, roomID, bob.UserID, client.WithKickEndpointVersion(client.KickRoomURLMSC4080))
	alice.MustSyncUntil(t, client.SyncReq{}, client.SyncLeftFrom(bob.UserID, roomID))
}

func TestKickedUserCanRejoin(t *testing.T) {
	deployment := complement.Deploy(t, 1)
	defer deployment.Destroy(t)
