...
func TestDelayedEvents(t *testing.T) {
	...
	t.Run("delayed events sent on timeout are finalised with their event ID", func(t *testing.T) {
		...
		res := user.MustDo(
			t,
			"PUT",
			getPathForSend(roomID, eventType, "txn-delayed-msg-finalised-timeout"),
	...
	t.Run("cancelled delayed events are finalised without an event ID", func(t *testing.T) {
		...
		res = user.MustDo(
			t,
			"PUT",
			getPathForSend(roomID, eventType, "txn-delayed-msg-finalised-cancel"),
			client.WithJSONBody(t, map[string]interface{}{}),
			getDelayQueryParam("100000"),
		)
		delayID := client.GetJSONFieldStr(t, client.ParseJSON(t, res), "delay_id")
	...
	t.Run("delayed events that fail to be sent are finalised with an error", func(t *testing.T) {
		...
		res := user2.MustDo(
			t,
			"PUT",
			getPathForState(failRoomID, eventType, stateKey),
			client.WithJSONBody(t, map[string]interface{}{}),
			getDelayQueryParam("1500"),
		)
		delayID := client.GetJSONFieldStr(t, client.ParseJSON(t, res), "delay_id")
	...
	for _, tc := range []struct {
		...
		t.Run(fmt.Sprintf("actions on %s delayed events succeed if repeated and conflict otherwise", tc.name), func(t *testing.T) {
			...
			res := user.MustDo(
				t,
				"PUT",
				getPathForSend(roomID, eventType, fmt.Sprintf("txn-delayed-msg-finalised-%s-actions", tc.name)),
				client.WithJSONBody(t, map[string]interface{}{}),
				getDelayQueryParam("100000"),
			)
			delayID := client.GetJSONFieldStr(t, client.ParseJSON(t, res), "delay_id")
	...
	t.Run("finalised delayed events are not listed", func(t *testing.T) {
		...
		for i := range delayIDs {
			res = user.MustDo(
				t,
				"PUT",
				getPathForSend(roomID, eventType, fmt.Sprintf("txn-delayed-msg-finalised-list-%d", i)),
				client.WithJSONBody(t, map[string]interface{}{}),
				getDelayQueryParam("100000"),
			)
